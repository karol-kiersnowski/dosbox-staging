...
{
	// Ensure test harness checks thread-correctness on scope closure
	...
	// When threads terminate, the OS takes time to remove them from the
	// process's thread pool. This removal unfortunately isn't guaranteed to
	// happen instantly (or block the main thread), so here we give the OS a
	// bit of time to remove them. Without a bit of wait time, Google Test
	// can sometimes catch their presence on this scope closure.
