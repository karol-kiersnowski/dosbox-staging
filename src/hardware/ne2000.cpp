...
void bx_ne2k_c::reset(unsigned type)
{
	...
	// memset( & BX_NE2K_THIS s.physaddr, 0, sizeof(BX_NE2K_THIS
	// s.physaddr)); memset( & BX_NE2K_THIS s.mchash, 0, sizeof(BX_NE2K_THIS
	// s.mchash));
...
//
uint32_t bx_ne2k_c::read_cr(void)
{
	uint32_t val = (((unsigned int)(BX_NE2K_THIS s.CR.pgsel & 0x03u) << 6u) |
...
void bx_ne2k_c::write_cr(uint32_t value)
{
	...
	if (BX_NE2K_THIS s.CR.rdma_cmd == 3) {
		...
		BX_NE2K_THIS s.remote_bytes = *(
		        (uint16_t *)&BX_NE2K_THIS s.mem[BX_NE2K_THIS s.bound_ptr * 256 +
		                                        2 - BX_NE2K_MEMSTART]);
		BX_INFO("Sending buffer #x%x length %d",
		        BX_NE2K_THIS s.remote_start, BX_NE2K_THIS s.remote_bytes);
	}
	...
	if ((value & 0x04) && BX_NE2K_THIS s.TCR.loop_cntl) {
		...
		if (BX_NE2K_THIS s.TCR.loop_cntl != 1) {
			...
			// do a TX interrupt
			// Generate an interrupt if not masked and not one in
			// progress
		...
		for (int i = 0; i < BX_NE2K_THIS s.tx_bytes; i++) {
			printf("%02x ",
			       BX_NE2K_THIS s.mem[BX_NE2K_THIS s.tx_page_start * 256 -
			                          BX_NE2K_MEMSTART + i]);
			if (i && (((i + 1) % 16) == 0))
				printf("\t");
		}
		...
		// s.tx_bytes*8)/10;
		s.tx_timer_active = 1;
		PIC_AddEvent(NE2000_TX_Event,
		             (float)((64 + 96 + 4 * 8 + BX_NE2K_THIS s.tx_bytes * 8) /
		                     10000.0),
		             0);
	    ...
	    bx_pc_system.activate_timer(BX_NE2K_THIS s.tx_timer_index,
		                        (64 + 96 + 4*8 + BX_NE2K_THIS
	    s.tx_bytes*8)/10, 0); // not continuous
		*/
	...
	// Detect this here
	if (BX_NE2K_THIS s.CR.rdma_cmd == 0x01 && BX_NE2K_THIS s.CR.start &&
	    BX_NE2K_THIS s.remote_bytes == 0) {
...
uint32_t bx_ne2k_c::chipmem_read(uint32_t address, unsigned int io_len)
{
	...
	if (/*(address >=0) && */ address <= 31) {
		...
		if ((io_len == 2u) || (io_len == 4u)) {
			retval |= (unsigned int)(BX_NE2K_THIS s.macaddr[address + 1u]
			                         << 8u);
			if (io_len == 4u) {
				retval |= (unsigned int)(BX_NE2K_THIS s.macaddr[address + 2u]
				                         << 16u);
				retval |= (unsigned int)(BX_NE2K_THIS s.macaddr[address + 3u]
				                         << 24u);
			}
		}
		return (retval);
	...
	if ((address >= BX_NE2K_MEMSTART) && (address < BX_NE2K_MEMEND)) {
		...
		if ((io_len == 2u) || (io_len == 4u)) {
			retval |= (unsigned int)(BX_NE2K_THIS s.mem[address - BX_NE2K_MEMSTART + 1]
			                         << 8u);
		}
		if (io_len == 4u) {
			retval |= (unsigned int)(BX_NE2K_THIS s.mem[address - BX_NE2K_MEMSTART + 2]
			                         << 16u);
			retval |= (unsigned int)(BX_NE2K_THIS s.mem[address - BX_NE2K_MEMSTART + 3]
			                         << 24u);
		}
		return (retval);
	}
...
void bx_ne2k_c::chipmem_write(uint32_t address, uint32_t value, unsigned io_len)
{
	if ((io_len == 2) && (address & 0x1))
...
//
uint32_t bx_ne2k_c::asic_read(uint32_t offset, unsigned int io_len)
{
	uint32_t retval = 0;
	...
	case 0x0: // Data register
		...
		if (io_len > BX_NE2K_THIS s.remote_bytes) {
			BX_ERROR("ne2K: dma read underrun iolen=%d remote_bytes=%d",
			         io_len, BX_NE2K_THIS s.remote_bytes);
			// return 0;
		}

		// BX_INFO(("ne2k read DMA: addr=%4x
		// remote_bytes=%d",BX_NE2K_THIS s.remote_dma,BX_NE2K_THIS
		// s.remote_bytes));
...
void bx_ne2k_c::asic_write(uint32_t offset, uint32_t value, unsigned io_len)
{
	BX_DEBUG("asic write addr=0x%02x, value=0x%04x", (unsigned)offset,
	         (unsigned)value);
	switch (offset) {
...
//
uint32_t bx_ne2k_c::page0_read(uint32_t offset, unsigned int io_len)
{
	BX_DEBUG("page 0 read from port %04x, len=%u", (unsigned)offset,
	         (unsigned)io_len);
	if (io_len > 1) {
		BX_ERROR("bad length! page 0 read from port %04x, len=%u",
	...
	case 0x4: // TSR
		return ((unsigned int)(BX_NE2K_THIS s.TSR.ow_coll << 7u) |
	...
	case 0x7: // ISR
		return ((unsigned int)(BX_NE2K_THIS s.ISR.reset << 7u) |
	...
	case 0xc: // RSR
		return ((unsigned int)(BX_NE2K_THIS s.RSR.deferred << 7u) |
...
void bx_ne2k_c::page0_write(uint32_t offset, uint32_t value, unsigned io_len)
{
	...
	case 0xd: // TCR
		...
		if (value & 0x06) {
			...
			BX_INFO("TCR write, loop mode %d not supported",
			        BX_NE2K_THIS s.TCR.loop_cntl);
		} else {
	...
	case 0xe: // DCR
		...
		BX_NE2K_THIS s.DCR.auto_rx = ((value & 0x10) == 0x10); // also
		                                                       // illegal ?
...
//
uint32_t bx_ne2k_c::page1_read(uint32_t offset, unsigned int io_len)
{
...
void bx_ne2k_c::page1_write(uint32_t offset, uint32_t value, unsigned io_len)
{
	(void)io_len; // UNUSED
	...
	case 0x4:
	case 0x5:
	case 0x6: BX_NE2K_THIS s.physaddr[offset - 1] = value; break;
...
//
uint32_t bx_ne2k_c::page2_read(uint32_t offset, unsigned int io_len)
{
	BX_DEBUG("page 2 read from port %04x, len=%u", (unsigned)offset,
	         (unsigned)io_len);
	...
	case 0xc: // RCR
		return ((unsigned int)(BX_NE2K_THIS s.RCR.monitor << 5u) |
	...
	case 0xd: // TCR
		return ((unsigned int)(BX_NE2K_THIS s.TCR.coll_prio << 4u) |
	...
	case 0xe: // DCR
		return (((unsigned int)(BX_NE2K_THIS s.DCR.fifo_size & 0x3) << 5u) |
	...
	case 0xf: // IMR
		return ((unsigned int)(BX_NE2K_THIS s.IMR.rdma_inte << 6u) |
		        (unsigned int)(BX_NE2K_THIS s.IMR.cofl_inte << 5u) |
		        (unsigned int)(BX_NE2K_THIS s.IMR.overw_inte << 4u) |
		        (unsigned int)(BX_NE2K_THIS s.IMR.txerr_inte << 3u) |
		        (unsigned int)(BX_NE2K_THIS s.IMR.rxerr_inte << 2u) |
...
void bx_ne2k_c::page2_write(uint32_t offset, uint32_t value, unsigned io_len)
{
	(void)io_len; // UNUSED
...
//
uint32_t bx_ne2k_c::page3_read(uint32_t offset, unsigned int io_len)
{
	(void)offset; // UNUSED
	(void)io_len; // UNUSED
...
void bx_ne2k_c::page3_write(uint32_t offset, uint32_t value, unsigned io_len)
{
	(void)value;  // UNUSED
	(void)offset; // UNUSED
	(void)io_len; // UNUSED
...
uint32_t bx_ne2k_c::read(uint32_t address, unsigned io_len)
{
	...
	uint32_t retval = 0;
	unsigned int offset = (unsigned int)address -
	...
	if (offset >= 0x10) {
		...
		case 0x00: retval = page0_read(offset, io_len); break;

		case 0x01: retval = page1_read(offset, io_len); break;

		case 0x02: retval = page2_read(offset, io_len); break;
...
// by this ne2000 instance
//
void bx_ne2k_c::write_handler(void *this_ptr, uint32_t address, uint32_t value, unsigned io_len)
{
#if !BX_USE_NE2K_SMF
	bx_ne2k_c *class_ptr = (bx_ne2k_c *)this_ptr;
...
void bx_ne2k_c::write(uint32_t address, uint32_t value, unsigned io_len)
{
#else
	UNUSED(this_ptr);
#endif // !BX_USE_NE2K_SMF
	...
	if (offset >= 0x10) {
		...
		switch (BX_NE2K_THIS s.CR.pgsel) {
		case 0x00: page0_write(offset, value, io_len); break;

		case 0x01: page1_write(offset, value, io_len); break;

		case 0x02: page2_write(offset, value, io_len); break;
...
#define POLYNOMIAL 0x04c11db6
	...
	return (uint32_t)((crc & 0xfffffffful) >> 26ul); /* WARNING: Caller
	                                                    directly uses our
	                                                    6-bit return as
	                                                    index. If not
	                                                    truncated, will
...
void bx_ne2k_c::rx_frame(const void *buf, unsigned io_len)
{
	...
	static unsigned char bcast_addr[6] = {0xff, 0xff, 0xff,
	                                      0xff, 0xff, 0xff};
	...
	BX_INFO("rx_frame %d to %x:%x:%x:%x:%x:%x from %x:%x:%x:%x:%x:%x",
	        io_len, pktbuf[0], pktbuf[1], pktbuf[2], pktbuf[3], pktbuf[4],
	        pktbuf[5], pktbuf[6], pktbuf[7], pktbuf[8], pktbuf[9],
	        pktbuf[10], pktbuf[11]);
	...
	startptr = &BX_NE2K_THIS s.mem[BX_NE2K_THIS s.curr_page * 256 - BX_NE2K_MEMSTART];
	if ((nextpage > BX_NE2K_THIS s.curr_page) ||
		...
		unsigned int endbytes = (unsigned int)(BX_NE2K_THIS s.page_stop -
		                                       BX_NE2K_THIS s.curr_page) *
		                        256u;
		memcpy(startptr, pkthdr, 4);
		memcpy(startptr + 4, buf, (size_t)(endbytes - 4u));
		startptr = &BX_NE2K_THIS s.mem[BX_NE2K_THIS s.page_start * 256u - BX_NE2K_MEMSTART];
...
void bx_ne2k_c::init()
{
	// BX_DEBUG(("Init $Id: ne2k.cc,v 1.56.2.1 2004/02/02 22:37:22 cbothamy
	// Exp $"));
	...
	// DEV_register_ioread_handler(this, read_handler, addr, "ne2000 NIC",
	// 3); DEV_register_iowrite_handler(this, write_handler, addr, "ne2000
	// NIC", 3);

	BX_INFO("port 0x%x/32 irq %d mac %02x:%02x:%02x:%02x:%02x:%02x",
	        (unsigned int)(BX_NE2K_THIS s.base_address),
	        (int)(BX_NE2K_THIS s.base_irq), BX_NE2K_THIS s.physaddr[0],
	        BX_NE2K_THIS s.physaddr[1], BX_NE2K_THIS s.physaddr[2],
	        BX_NE2K_THIS s.physaddr[3], BX_NE2K_THIS s.physaddr[4],
...
{
	ethernet->GetPackets([](uint8_t *packet, int len) {
		...
		if ((theNE2kDevice->s.DCR.loop == 0) ||
			...
			LOG_MSG("NE2000: skipping GetPackets because in loopback mode");
			return;
		}
...
public:
	...
	NE2K(Section *configuration) : Module_base(configuration)
	{
		...
		ethernet = OpenEthernetConnection("slirp");
		if (!ethernet) {
			load_success = false;
			return;
		}
		...
		if (!(irq == 3 || irq == 4 || irq == 5 || irq == 6 ||
		      irq == 7 || irq == 9 || irq == 10 || irq == 11 ||
		      irq == 12 || irq == 14 || irq == 15)) {
			irq = 3;
		}
		Bitu base = (Bitu)section->Get_hex("nicbase");
		if (!(base == 0x260 || base == 0x280 || base == 0x300 ||
		      base == 0x320 || base == 0x340 || base == 0x380)) {
			base = 0x300;
		}

		LOG_MSG("NE2000: Base=0x%x irq=%u", (unsigned int)base,
		        (unsigned int)irq);
